...
　
### 5-2. ロータリーエンコーダーRT8
　ロータリーエンコーダーのRT8は画面の表示内容を変更します。右に回すと次のページへ、左に回すと手前のページに移動します。ページは以下の通りです。  

|ページ|内容|
|---|---|
|SOUND MAIN|現在の音色の情報を表示します。|
...
|FILTER ADSR|フィルターエンベロープのADRSを設定します。|
|VCA|VCAのエンベロープを設定します。|
|SAVE|音色を保存します。|
|LOAD|音色を読み込みます。|
|SAMPLING|音をサンプリングして保存します。|
